-- Дайджест прихода: видео отметок приходят старшим в Telegram сами.
--
-- Видео к отметке обязательно уже давно, но открыть его — это зайти в «Люди»,
-- выбрать дату, найти человека и нажать 🎥. Никто так не делает, и 3 сентября
-- двое отметились не на месте: доказательство лежало в базе, но его никто не
-- смотрел. Теперь наоборот: ролики сами приезжают в Telegram альбомом, и
-- пролистать смену — это двадцать секунд.
--
-- Два выпуска в день, чтобы вечерняя смена не ждала до завтра:
--   день  — отметки до 16:00, уходит в 16:00
--   вечер — отметки с 16:00,  уходит в 23:30
-- Окна не пересекаются, поэтому один ролик не придёт дважды.
--
-- Ролики уходят БАЙТАМИ через send-telegram: ссылку Telegram скачать не может,
-- хранилище за Cloudflare отдаёт его фетчеру отлуп.

create table if not exists public.checkin_digest_notices (
  ...
  unique (date, part, filial, recipient)
);

alter table public.checkin_digest_notices enable row level security;
-- Таблица служебная: пишет её только функция (security definer), людям не нужна
...
begin
  ...
  foreach f in array array['chekhov','istikbol'] loop
    ...
    -- Заголовок: сколько отметок и кто пришёл без видео — таких видно отдельно
    ...
    loop
      ...
      continue when not found;   -- этот выпуск ему уже уходил
...
-- 11:00 UTC = 16:00 Ташкент · 18:30 UTC = 23:30 Ташкент
select cron.unschedule('checkin-digest-day')   where exists (select 1 from cron.job where jobname='checkin-digest-day');
select cron.unschedule('checkin-digest-night') where exists (select 1 from cron.job where jobname='checkin-digest-night');
select cron.schedule('checkin-digest-day',   '0 11 * * *',  $cron$select public.checkin_digest('day');$cron$);
select cron.schedule('checkin-digest-night', '30 18 * * *', $cron$select public.checkin_digest('night');$cron$);
