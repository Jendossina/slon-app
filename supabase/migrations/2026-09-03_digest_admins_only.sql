-- Дайджест прихода — только управляющим.
--
-- Было: уходил всем старшим филиала (notify_chiefs — управляющие, менеджеры,
-- владелец), сегодня это восемь человек. Стало: только управляющие. Видео
-- смены — материал для разбора, и расходиться шире, чем нужно, он не должен.
--
-- Роль, а не список имён: появится новый управляющий — начнёт получать сам.
...
begin
  ...
  foreach f in array array['chekhov','istikbol'] loop
    ...
    -- Только управляющие. Дайджест — инструмент разбора, а не общая рассылка:
    -- менеджеры и владелец из него убраны по решению владельца, чтобы ролики
    -- смены не расходились шире, чем нужно для проверки.
