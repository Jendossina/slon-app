...
test('смены подставляются по филиалу, а не общие', async ({ page }) => {
  ...
  const r = await page.evaluate(() => {
    ...
    return {
      ...
      emptyDepts: ['chekhov', 'istikbol'].flatMap(f =>
        ['Официанты', 'Бармены', 'Повара', 'Кальянные мастера']
          .filter(d => snapshot(f, d).length === 0)
          .map(d => f + '/' + d)),
    };
  });
  ...
  // Цех со своим списком не должен получить чужие часы: по ним считаются сроки
  // чек-листов, и подставленные соседские смены молча сдвинули бы штрафы.
  expect(times(r.istikbolCooks), 'у поваров Истикбола своя вечерняя смена').toContain('15:00-02:00');
  expect(times(r.chekhovCooks), 'а у поваров Чехова — своя').toContain('14:30-02:30');
  expect(times(r.istikbolCooks), 'чеховская повару Истикбола не подставилась').not.toContain('14:30-02:30');
  expect(r.istikbolCooks.find(x => x.label)?.label, 'поздний день поваров Истикбола — воскресенье').toBe('sch.presetSunday');

  // Интерфейс «Заполнить неделю» переключается на выпадающие списки только если
  // у цеха есть смены. Пока Истикбол знал только официантов, бар и кухня там
  // заполнялись руками, а в Чехове — списком: один и тот же экран выглядел
  // по-разному в зависимости от филиала.
  expect(r.emptyDepts, 'ни один цех не остался без смен').toEqual([]);
});
...
test('старший цеха ставит задачи своим, линейный сотрудник — никому', async ({ page }) => {
  ...
  // Владелец — наблюдатель во всём, кроме поручений: это его работа, а не
  // правка чужих записей. Отмечать выполнение и править данные он не может.
  expect(r.boss.can, 'владелец ставит задачи').toBe(true);
  expect(r.boss.fab, 'и видит кнопку «+»').toBe('flex');
  expect(r.boss.lead, 'но цехом не заведует').toBe(null);
});
