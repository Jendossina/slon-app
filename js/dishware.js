    ...
    html += items.map(it=>{
      ...
      return `<div class="card" style="display:flex;align-items:center;gap:12px">
        <div style="flex:1;cursor:pointer" onclick="openDishwareHistory(${it.id})">
          ...
          <div style="font-size:13px;color:${low?'#A32D2D':'var(--text-muted)'}">${t('inv.remaining')} <b>${dishwareFmt(q)} ${t('dish.pcs')}</b>${dishwareCanManage()?` · ${formatNum(it.cost)} ${t('dish.perPcs')}`:''}</div>
        </div>
        <button onclick="openDishwareBreak(${it.id})" style="background:#FCEBEB;color:#A32D2D;border:none;border-radius:8px;padding:8px 12px;font-size:13px;font-weight:600;cursor:pointer">${t('dish.breakBtn')}</button>
        ${dishwareCanManage()?`<button onclick="openDishwareIn(${it.id})" style="background:#e2efda;color:#3B6D11;border:none;border-radius:8px;padding:8px 12px;font-size:13px;font-weight:600;cursor:pointer">${t('inv.inBtn')}</button>`:''}
      </div>`;
    }).join('');
...
async function openDishwareHistory(itemId) {
  ...
  try {
    ...
    if(!moves || moves.length===0) { body.innerHTML=`<div class="empty"><div class="empty-text">${t('inv.noMoves')}</div></div>`; return; }
...
async function runDishwareReport() {
  ...
  try {
    ...
    const itemRows = Object.keys(byItem).map(id=>({name:itemMap[id]||t('inv.deleted'),...byItem[id]})).sort((a,b)=>b.loss-a.loss);
    ...
    res.innerHTML = `
      ...
      <div class="card">
        ...
        ${itemRows.map(r=>`<div class="list-item"><div class="item-info"><div class="item-name">${escapeHtml(r.name)}</div><div class="item-sub">${dishwareFmt(r.qty)} ${t('dish.pcs')}</div></div><div style="font-weight:700;color:#A32D2D;white-space:nowrap">−${formatNum(Math.round(r.loss))}</div></div>`).join('')}
      </div>
      <div class="card">
