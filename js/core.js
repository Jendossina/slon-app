...
// ===== СТАРШИЙ ЦЕХА =====
// Отдельного «системного» уровня для старших нет и не нужно: права даёт
// должность (employees.role) вместе с отделом. Старший — полный хозяин внутри
// своего цеха и никто за его пределами: ведёт график, правит карточки своих
// людей, пишет статьи в книгу своего цеха. Финансы, зарплаты, учётные записи и
// чужие отделы остаются руководству.
// Ключ — отдел, значение — должности, а НЕ системные роли.
// «Официанты» здесь намеренно нет: ими занимается руководство.
// Тот же список продублирован в базе (is_dept_lead, can_edit_schedule_of) —
...
// ===== Кто решает по заявкам сотрудника =====
// Отдельная лестница от DEPT_LEADS и вот почему: график и карточки своих людей
// ведут все старшие цеха, а решать по заявкам (опоздание, замена) — только шеф
// цеха. Кальянщиков решает исключительно управляющий, официантов — менеджер.
// Ключ — отдел заявки, значение — должности, которым это можно.
// Тот же список продублирован в базе (can_decide_request_of) — обязаны совпадать.
const REQUEST_APPROVERS = {
  'Бармены':            ['Шеф бармен'],
  'Повара':             ['Шеф повар'],
  'Официанты':          [],   // им хватает менеджера, отдельного шефа нет
  'Кальянные мастера':  [],   // намеренно пусто: только управляющий
};
// Может ли текущий пользователь решать по заявкам этого отдела
function canApproveDept(dept) {
  if(isAdmin()) return true;                            // управляющий — любой отдел
  if(isManager()) return dept === 'Официанты';          // менеджер — только зал
  const me = currentEmployee;
  if(!me || !dept || me.department !== dept) return false;
  return (REQUEST_APPROVERS[dept] || []).includes(me.role);
}
// Есть ли у него вообще право решать хоть по кому-то — нужно, чтобы решить,
// показывать ли раздел с заявками
function canApproveAny() {
  return isAdmin() || isManager() || Object.keys(REQUEST_APPROVERS).some(canApproveDept);
}

// Отдел, который ведёт текущий пользователь по должности (или null)
function myLeadDept() {
  const dept = currentEmployee?.department;
  if(!dept || isBoss()) return null;
  return (DEPT_LEADS[dept] || []).includes(currentEmployee.role) ? dept : null;
}
// Старший цеха — тот, у кого прав больше рядового, но меньше руководства
function isDeptLead() { return !canEditData() && !!myLeadDept(); }
// Можно ли вести этот отдел: руководство — любой, старший — только свой
function canLeadDept(dept) {
...
// Гео-проверка отметки прихода убрана: на части Android-телефонов геолокация
// не работает даже в других приложениях, и такие сотрудники не могли отметиться.
// Место подтверждает обязательное видео с камеры.
...
// «Выйти» — всегда полный выход к вводу пароля.
async function doLogout() {
  await fullLogout();
}

async function fullLogout() {
  // signOut идёт в сеть. Раньше его ждали без защиты, и при плохой связи выход
...
// Убираем следы прежнего входа по отпечатку: фича снята, ключи в памяти
// браузера остались бы мусором на каждом телефоне.
(function cleanupOldBiometricKeys() {
  try { ['slon-bio-enabled','slon-bio-credid','slon-bio-user'].forEach(k => localStorage.removeItem(k)); }
  catch(e) {}
})();
