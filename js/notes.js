...
function updateMyNoteFields() {
  ...
  if(kind==='note') { dueGroup.style.display='none'; label.textContent='Текст заметки'; }
  else { dueGroup.style.display=''; label.textContent='Что нужно сделать'; }
...
function openMyNoteModal(id) {
  ...
  if(id) {
    document.getElementById('mynote-modal-title').textContent = 'Редактировать';
    ...
    document.getElementById('mynote-modal-title').textContent = 'Новая запись';
...
async function loadMyNotes() {
  ...
  try {
    ...
    if(notes.length===0) {
      content.innerHTML = '<div class="card"><div class="empty"><div class="empty-icon">📝</div><div class="empty-text">Пусто. Нажми «+ Добавить».</div></div></div>';
      return;
    }
    ...
    content.innerHTML = notes.map(n=>{
      ...
      return `<div class="card" style="${imp?'border-left:3px solid #A32D2D;':''}${n.is_done?'opacity:0.55;':''}display:flex;align-items:start;gap:10px">
        ...
        <div style="flex:1;cursor:pointer" onclick="openMyNoteModal(${n.id})">
          ...
          ${n.due_date?`<div style="font-size:12px;color:${overdue?'#A32D2D':'var(--text-muted)'};margin-top:4px">${overdue?'⚠️ просрочено · ':'📅 '}${dueLabel}</div>`:''}
        </div>
      </div>`;
    }).join('');
  } catch(e) { content.innerHTML = '<div class="card"><div class="empty"><div class="empty-text">Ошибка. Возможно, задачник ещё не настроен в Supabase.</div></div></div>'; }
}
