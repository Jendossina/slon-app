...
const DEPARTMENTS = ['Официанты','Бармены','Кальянные мастера','Повара','Менеджеры'];

// Готовые варианты смен по цехам — управляющий выбирает кнопкой, а не вводит время вручную.
// end <= start означает переход через полночь (напр. 15:00–03:00). Цеха без списка
...
const SHIFT_PRESETS = {
  ...
  // Времена сняты с графика, который уже отработан на филиале, а не перенесены из
  // Чехова: они там другие (повара 15:00–02:00 против 14:30–02:30, бар до 23:00
  // против полуночи). По этим временам считаются сроки чек-листов, и чужие часы
  // молча сдвинули бы штрафы.
  istikbol: {
    // Смены 18:00 здесь нет. Позднее открытие — воскресенье, 15:00: штат
    // выходит в 13:00 и работает до закрытия, поэтому это отдельная смена, а не
    // помеченная обычная. Дневная 13:00–01:00 остаётся сама по себе.
    'Официанты': [
      { start: '11:00', end: '23:00' },
      { start: '13:00', end: '01:00' },
      { start: '15:00', end: '03:00' },
      { start: '13:00', end: '03:00', label: 'sch.presetSunday' },
    ],
    'Бармены': [
      { start: '11:30', end: '23:00' },
      { start: '15:00', end: '02:00' },
      { start: '11:30', end: '02:00', full: true }, // весь день
      { start: '13:00', end: '02:00', label: 'sch.presetSunday' },
    ],
    'Кальянные мастера': [
      { start: '11:30', end: '00:00' },
      { start: '12:45', end: '01:00' },
  ...
  try {
    ...
    const [empsR, schedR] = await Promise.all([
      cachedEmps
        ...
        // пустыми строками.
                // Уволенных в сетке быть не должно: in_schedule у них остаётся
        // прежним (он про «работает ли человек по сменам вообще»), и без
        // этой проверки уволенный висел пустой строкой каждую неделю.
        : sb.from('employees_view').select('id,name,filials,in_schedule,status').eq('department', dept).eq('in_schedule', true).neq('status', 'Уволен').order('name'),
...
async function openWeekFillPicker() {
  const { data: allEmps } = await sb.from('employees').select('id,name,filials').eq('department', currentDept).eq('in_schedule', true).neq('status', 'Уволен').order('name');
